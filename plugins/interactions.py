...
def _cocoa_ts(value):
    """Convert CFAbsoluteTime (seconds since 2001-01-01) to datetime.
    Returns None for null/zero instead of emitting epoch-zero (2001-01-01),
    so contacts/interactions with no activity on a given side don't
    pollute the timeline with fake 2001 timestamps."""
            ...
            try:
                ...
                for row in cursor:
                    ...
                    yield InteractionRecord(
                        ...
                        # ZTARGETBUNDLEID exists in the schema but is null on
                        # every observed macOS 14/15 install. Kept for forward
                        # compatibility if Apple starts populating it.
