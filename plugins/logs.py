...
# Sanity window for ASL records. Apple platforms predating 2007 are vanishingly
# rare in DFIR; anything more than ~18 months in the future is clock drift or a
# corrupted record. Records outside this window are dropped.
_ASL_TS_MIN = datetime(2007, 1, 1, tzinfo=timezone.utc)
_ASL_TS_MAX = datetime.now(timezone.utc) + timedelta(days=548)
...
def _parse_asl_file(data):
    ...
    while pos < len(data) - 60:
        ...
        # Sanity check — ASL records occasionally get corrupted on disk
        # (truncation, partial writes). Real syslog levels are 0–7 and real
        # timestamps should fall inside a plausible forensic window; anything
        # else is a garbled record that would otherwise poison the timeline.
        if level > 7:
            pos += rec_len + 2
            continue
        if ts is not None and not (_ASL_TS_MIN <= ts <= _ASL_TS_MAX):
            pos += rec_len + 2
            continue
