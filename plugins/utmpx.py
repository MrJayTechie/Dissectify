...
# Apple utmpx(5) entry types — see /usr/include/utmpx.h
# SIGNATURE (10) marks the file header row ("utmpx-1.00"), not a user session.
UTMPX_SIGNATURE = 10
        ...
        for utmpx_path in self._utmpx_paths:
            try:
                ...
                while offset + UTMPX_RECORD_SIZE <= len(data):
                    ...
                    # Skip the file signature record (user="utmpx-1.00",
                    # entry_type=SIGNATURE). Not a real session.
                    if entry_type == UTMPX_SIGNATURE or user.startswith("utmpx-"):
