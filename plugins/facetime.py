...
class FaceTimePlugin(Plugin):
    ...
    def _parse_links(self, db_path):
        ...
        try:
            ...
            for row in cursor:
                raw_name = row["ZNAME"] if "ZNAME" in columns else None
                pseudonym = row["ZPSEUDONYM"] if "ZPSEUDONYM" in columns and row["ZPSEUDONYM"] else ""
                # Users rarely give FaceTime links a custom name, so ZNAME
                # is almost always NULL. Fall back to the pseudonym (the
                # 8-char invite code that identifies the link) so the
                # record is still humanly identifiable in a timeline.
                link_name = raw_name or (f"link-{pseudonym}" if pseudonym else "")
