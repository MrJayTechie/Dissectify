        ...
        for browser_name, db_path, conn, tmp in self._iter_db("Login Data"):
            try:
                ...
                for row in cursor:
                    ...
                    # Chromium uses WebKit-epoch zero (1601-01-01) as the
                    # "never used" sentinel. Emit NULL instead so these
                    # credentials don't sort at year 1601 on the timeline.
                    ts_last_used = _chromium_ts(row["date_last_used"]) if times_used else None
                    yield ChromiumLoginRecord(
                        ts_created=_chromium_ts(row["date_created"]),
                        ts_last_used=ts_last_used,
