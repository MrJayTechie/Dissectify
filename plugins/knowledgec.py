...
def _cocoa_ts(value):
    """CFAbsoluteTime (seconds since 2001-01-01) → datetime. Returns None
    on null/zero so rows without a real timestamp (e.g. sync_peers never
    seen, metadata stubs) don't appear at 2001-01-01."""
...
class KnowledgeCPlugin(Plugin):
    ...
    def notifications(self) -> Iterator[KnowledgeCNotificationRecord]:
        ...
        for db_path, conn, tmp in self._iter_db():
            try:
                cursor = conn.cursor()
                # For /notification/usage the bundle id is NOT in ZSOURCE
                # (that FK is null) — it lives in ZSTRUCTUREDMETADATA under
                # the `_BUNDLEID` key, typically prefixed with
                # "_SYSTEM_CENTER_:" which we strip.
                cursor.execute("""
                    SELECT o.ZSTARTDATE, o.ZENDDATE, o.ZCREATIONDATE,
                           o.ZVALUESTRING AS event_value,
                           sm.Z_DKNOTIFICATIONUSAGEMETADATAKEY__BUNDLEID AS bundle_id
                    FROM ZOBJECT o
                    LEFT JOIN ZSTRUCTUREDMETADATA sm ON o.ZSTRUCTUREDMETADATA = sm.Z_PK
                ...
                """)
                for row in cursor:
                    raw_bundle = row["bundle_id"] or ""
                    if ":" in raw_bundle:
                        raw_bundle = raw_bundle.rsplit(":", 1)[-1]
    ...
    def sources(self) -> Iterator[KnowledgeCSourceRecord]:
        """Parse registered event sources from knowledgeC.db. Dedupes by
        bundle id and drops rows where every non-bundle column is NULL —
        ZSOURCE is internal plumbing and emitting every row with only a
        bundle id produces thousands of noise records."""
        for db_path, conn, tmp in self._iter_db():
            try:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT DISTINCT ZBUNDLEID, ZSOURCEID, ZDEVICEID,
                                    ZGROUPID, ZITEMID
                    FROM ZSOURCE
                ...
                seen = set()
                for row in cursor:
                    bundle = row["ZBUNDLEID"] or ""
                    src_id = row["ZSOURCEID"]
                    dev_id = row["ZDEVICEID"]
                    grp_id = row["ZGROUPID"]
                    itm_id = row["ZITEMID"]
                    # If every identifier besides bundle is NULL, only emit
                    # the bundle once — saves ~1300 duplicate noise rows.
                    if not (src_id or dev_id or grp_id or itm_id):
                        if bundle in seen:
                            continue
                        seen.add(bundle)
    ...
    def custom_metadata(self) -> Iterator[KnowledgeCCustomMetadataRecord]:
        """Parse custom metadata entries from knowledgeC.db. Rows where every
        value column (string/int/double/date) is empty are skipped — those
        are placeholder metadata stubs with the payload stored elsewhere
        (typically a serialized key-image blob)."""
        for db_path, conn, tmp in self._iter_db():
            try:
                cursor = conn.cursor()
                cursor.execute("""
                    ...
                    WHERE ZSTRINGVALUE IS NOT NULL
                       OR ZINTEGERVALUE IS NOT NULL
                       OR ZDOUBLEVALUE IS NOT NULL
                       OR ZDATEVALUE IS NOT NULL
                    ORDER BY ZOBJECT
                """)
                for row in cursor:
