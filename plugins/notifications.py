...
def _cocoa_ts(value):
    """Convert a Cocoa CFAbsoluteTime (seconds since 2001-01-01) to datetime.
    Returns None for null/zero so the timeline treats it as missing rather
    than pinning the record to 2001."""
...
def _clean_bundle_id(raw):
    """Strip the usernoted scope prefix from composite bundle ids like
    '_system_center_:com.apple.foo' → 'com.apple.foo'."""
    if not raw:
        return ""
            ...
            try:
                ...
                for row in cursor:
                    yield NotificationEntryRecord(
                        ...
                        bundle_id=_clean_bundle_id(row["identifier"]),
