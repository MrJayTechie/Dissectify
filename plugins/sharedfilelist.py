        ...
        for path in self._paths:
            ...
            try:
                for entry in self._parse_sfl3(path):
                    # Skip container-header rows: no resolved path and no
                    # useful bookmark payload (bookmark_strings is empty or
                    # just a scheme placeholder like "file:///").
                    if not entry["item_path"]:
                        strings = entry["strings"] or []
                        placeholder = all(
                            (not s) or s == "file:///" for s in strings
                        )
                        if placeholder:
                            continue
